    ...
    public class SimpleBlitFeature : ScriptableRendererFeature
{
    ...
    public bool drawnInEditor;
    
    private SimpleBlitFeaturePass fullScreenPass;
    ...
    {
        fullScreenPass = new SimpleBlitFeaturePass();
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        if (renderingData.cameraData.camera.cameraType == CameraType.SceneView) return;
        fullScreenPass.Setup(passMaterial, passIndex, requiresColor, injectedBeforeTransparents, "SimpleBlitFeaturePass", renderingData);
    ...
    class SimpleBlitFeaturePass : ScriptableRenderPass
    {
        ...
        private static void ExecutePass(PassData passData, ref RenderingData renderingData, ref ScriptableRenderContext context)
        {
            ...
            using (new ProfilingScope(cmd, profilingSampler))
            {
                if (requiresColor)
                {
                    ...
                    var source = isBeforeTransparents ? null : cameraData.renderer.cameraColorTargetHandle;

                    Blitter.BlitCameraTexture(cmd, source, copiedColor);
                    passMaterial.SetTexture(m_BlitTextureShaderID, copiedColor);
                }

                CoreUtils.SetRenderTarget(cmd, null);
